...
class MooGrammar(Grammar):
  ...
  SUBSCRIPT = Ref()
  EXPRESSION = Ref()
  VALUE = Choice(r_int, r_objnum, r_string, r_float, LIST, MAP, VERB_CALL, FUNCTION_CALL, PROP_REF, r_var)
  ...
  CALL_ARGS = List(Choice(EXPRESSION))
  FUNCTION_CALL = Sequence(r_var, t_lparen, Optional(CALL_ARGS), t_rparen)
  VERB_CALL = Sequence(r_var, t_colon, r_var, t_lparen, Optional(CALL_ARGS), t_rparen)
  ...
  STATEMENT = Sequence(Choice(RETURN, ASSIGNMENT, VERB_CALL, FUNCTION_CALL), t_semi)
  START = Repeat(Choice(CONDITIONAL, WHILE_LOOP, FOR_LOOP, STATEMENT, most_greedy=False))
